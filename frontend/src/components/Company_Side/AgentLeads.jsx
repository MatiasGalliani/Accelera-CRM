...
export default function LeadsAgenti() {
  ...
  const filteredLeads = (leads || []).filter(lead => {
    ...
    // For AIQuinto source, differentiate by subtype.
    if (currentSource === 'aiquinto') {
      // If no search term, filter based on the presence of fields distinguishing each subtype.
      if (!searchTerm) {
        if (aiquintoTab === AIQUINTO_TABS.DIPENDENTI) {
          return !!lead.stipendioNetto;
        } else if (aiquintoTab === AIQUINTO_TABS.PENSIONATI) {
          return !!lead.pensioneNettaMensile;
        }
      }

      // When search term is provided, match against subtype-specific fields.
      if (aiquintoTab === AIQUINTO_TABS.DIPENDENTI) {
        return (
