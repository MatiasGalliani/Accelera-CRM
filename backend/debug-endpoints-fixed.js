...
async function debugAgentLeads(firebaseUid, source) {
  try {
    ...
    if (leadCount > 0) {
      // Consultar directamente la base de datos para evitar problemas con los nombres de campos
      const [leads] = await sequelize.query(`
        SELECT id, source, first_name, last_name, email 
        FROM leads 
      ...
      leads.forEach(lead => {
        console.log(`- ID: ${lead.id}, Fuente: ${lead.source}, Nombre: ${lead.first_name} ${lead.last_name}, Email: ${lead.email}`);
      });
    }
  } catch (error) {
